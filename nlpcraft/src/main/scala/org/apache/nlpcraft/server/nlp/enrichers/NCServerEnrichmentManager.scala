...
object NCServerEnrichmentManager extends NCService with NCIgniteInstance {
    private object Config extends NCConfigurable {
        ...
        lazy val supportCtxWords: Boolean = hasProperty("nlpcraft.server.ctxword")
    }
    ...
    @volatile private var supportCtxWords: Boolean = true
    ...
    /**
      ...
      * @param ctxWordsCfg Context words model config holder. Optional.
    ...
    private def process(
        ...
        ctxWordsCfg: Option[NCContextWordConfigMdo],
        parent: Span = null
    ): NCNlpSentence =
        startScopedSpan("process", parent, "srvReqId" → srvReqId, "txt" → normTxt) { span ⇒
            val s = new NCNlpSentence(srvReqId, normTxt, 1, enabledBuiltInToks, ctxWordsCfg)
            ...
            if (Config.supportCtxWords && ctxWordsCfg.nonEmpty)
...
//                    case None ⇒
                        process(srvReqId, normTxt, enabledBuiltInToks, ctxWordsCfg, span)
//                }
            }
        }
    }
    ...
    override def start(parent: Span = null): NCService = startScopedSpan("start", parent) { span ⇒
        ...
        if (Config.supportCtxWords)
            ps += (() ⇒ NCContextWordEnricher.start(span))
        ...
        supportCtxWords = Config.supportCtxWords
    ...
    /**
      ...
      * @return
      */
    def isSupportCtxWords: Boolean = supportCtxWords
}
